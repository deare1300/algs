  ...
  class YouDao {
      ...
      private:
           ...
           */
          bool __isContain(BinTree<T> *tr1, BinTree<T> *tr2);
  
          /*
           *find the min length of the substring that contain all the chars in a set
           */
          int __minLength(const string &str, set<char> &sets);
             /*
           * second interview
           * three problems 
  ...
  template<class T>
  int YouDao<T>::__minLength(const string &str, unorderd_set<char> &sets) {
      int len = str.length();
      int sLen = sets.size();
      int pre = 0;
      int back = 0;
      unorderd_map<char, int> maps;
      int minLen = len+1;
      for(; pre < len; ++pre) {
